...
function App(): JSX.Element {
        ...
        async function fetchQuestions() {
            try {
                const response = await fetch(
                    `/api/questions?quizId=${quizSelect || "9"}`
                );
                if (!response.ok) {
    ...
    useEffect(() => {
        async function fetchTimelineContent() {
            ...
            try {
                ...
                setError(
                    "Failed to fetch timeline content. Please try again later."
                );
            }
        }
    ...
    function handleDragEnd(e: DragEndEvent) {
        ...
        if (over) {
            ...
            if (active.data.current?.container === "palette") {
                if (
                    over.data.current?.container === "palette" ||
                    over.id === "palette"
                ) {
                    ...
                    (over && over.data.current?.container === "contentArea") ||
                    over.id === "contentArea"
                ) {
            ...
            if (active.data.current?.container === "timeline") {
                if (
                    ...
                    over.data.current?.container === "contentArea" ||
                    over.id === "contentArea"
                ) {
    ...
    function handleDragOverlay() {
        let activeCard = questions.find(
            (content) => content.id === currentDragCard?.id
        );
        if (!activeCard) {
            activeCard = timelineContentList.find(
                (timelineContent) => timelineContent.id === currentDragCard?.id
            );
        }
        if (activeCard) {
            return (
